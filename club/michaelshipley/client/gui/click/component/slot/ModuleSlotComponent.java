...
package club.michaelshipley.client.gui.click.component.slot;

import java.util.Iterator;

import club.michaelshipley.client.gui.click.ClickGui;
import club.michaelshipley.client.gui.click.component.window.ModuleWindow;
import club.michaelshipley.client.gui.click.component.window.OptionWindow;
import club.michaelshipley.client.gui.click.component.window.Window;
import club.michaelshipley.client.module.Module;
import club.michaelshipley.client.module.modules.view.Gui;
...
public class ModuleSlotComponent extends SlotComponent<Module>
{
    ...
    public void draw(final int mouseX, final int mouseY) {
        ...
        RenderUtils.rectangleBordered(this.getX(), this.getY(), this.getX() + this.getWidth(), this.getY() + this.getHeight(), 0.5, RenderUtils.blend(useDarkTheme ? -14540254 : -13421773, -16777216, this.getParent().isEnabled() ? (this.hovering(mouseX, mouseY) ? 0.6f : 0.7f) : (this.hovering(mouseX, mouseY) ? 0.8f : 1.0f)), RenderUtils.blend(useDarkTheme ? -14540254 : -13421773, -16777216, this.getParent().isEnabled() ? (this.hovering(mouseX, mouseY) ? 0.6f : 0.7f) : (this.hovering(mouseX, mouseY) ? 0.8f : 1.0f)));
        ...
        ClientUtils.clientFont().drawStringWithShadow(this.getParent().getDisplayName(), this.getX() + 2, this.getY() + 0.5 + this.getHeight() / 2.0 - ClientUtils.clientFont().FONT_HEIGHT / 2, -1);
